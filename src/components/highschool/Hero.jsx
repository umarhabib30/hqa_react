...
const Hero = () => {
  ...
  const menuItems = [
    ...
    { text: "Elementary-School", link: "/elementary-school" },
    { text: "Middle-School", link: "/middle-school" },
  ...
  return (
    <div className="w-full flex flex-col items-center px-8 pt-12 font-serif">
      ...
      >
        ...
        <div className="hidden md:flex justify-center flex-wrap gap-34 text-center">
          {menuItems.map((item, i) => (
            <Link
              key={i}
              to={item.link}
              className="text-black hover:opacity-80 cursor-pointer text-[16px]"
            >
              {item.text}
            </Link>
          ))}
